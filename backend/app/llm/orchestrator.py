...
class AIOrchestrator:
    ...
    def run(self, req: AIQueryIn) -> AIResponse:
        ...
        if llm_result is not None:
            ...
            if llm_result.prompt_tokens:
                LLM_TOKENS.labels(self.provider.name, "prompt").inc(llm_result.prompt_tokens)
            if llm_result.completion_tokens:
                LLM_TOKENS.labels(self.provider.name, "completion").inc(llm_result.completion_tokens)
    ...
    def _finish(self, req, plan, ev, calls, timer, answer, method, llm_result, warnings, limitations, status,
        ...
        model = llm_result.model if llm_result else getattr(self.provider, "model", None)
        AI_QUERIES.labels(" + ".join(capabilities), status).inc()
        self._trace(req, capabilities, method, status, error_code, timer, ev, calls, used, llm_result, model)
        ...
        return AIResponse(
            ...
            provider=self.provider.name, model=model, stage_ms={**timer.stages, "total": timer.total_ms},
    ...
    def _trace(self, req, capabilities, method, status, error_code, timer, ev, calls, used, llm_result, model) -> None:
        try:
            self.db.add(AIQueryTrace(
                ...
                route=" + ".join(capabilities), routing_method=method, provider=self.provider.name,
