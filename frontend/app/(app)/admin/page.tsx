    ...
    <Card bodyClassName="p-0" title="User accounts" subtitle="Role changes are recorded in the audit log" actions={<Button size="sm" onClick={() => setCreating(true)}><UserPlus className="h-3.5 w-3.5" /> New user</Button>}>
      ...
      <DataTable rows={data} loading={loading} error={error} onRetry={reload}
        columns={[
          ...
          { key: "login", header: "Last sign-in", render: (u) => <span className="text-xs text-slate-500">{fmtDateTime(u.last_login_at)}</span> },
          { key: "act", header: "", render: (u) => u.id !== me?.id && (
            <button className="text-xs text-brand-700 hover:underline" onClick={() => update(u, { is_active: !u.is_active })}>{u.is_active ? "Deactivate" : "Activate"}</button>) },
        ]} />
    ...
    <Card bodyClassName="p-0" title="Permission matrix" subtitle="Row-level access (which patients) is enforced separately by the access policy">
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-slate-100 text-left text-[11px] uppercase text-slate-500"><th className="px-3 py-2">Permission</th>{data.map((r) => <th key={r.name} className="px-3 text-center">{r.name}</th>)}</tr></thead>
          <tbody className="divide-y divide-slate-100">
            {perms.map((p) => (
              <tr key={p.code}><td className="px-3 py-1.5"><div className="font-mono text-xs">{p.code}</div><div className="text-[11px] text-slate-500">{p.description}</div></td>
                {data.map((r) => <td key={r.name} className="text-center">{r.permissions.includes(p.code) ? <span className="text-brand-700">●</span> : <span className="text-slate-300">○</span>}</td>)}</tr>
            ))}
          </tbody>
        </table>
      </div>
    ...
    <Card bodyClassName="p-0" title="Audit trail" subtitle="Identifiers and counts only — no clinical text or query content is stored">
      ...
      <DataTable dense rows={data?.items} loading={loading} error={error} onRetry={reload}
        columns={[
          ...
          { key: "res", header: "Resource", render: (l) => <span className="text-xs text-slate-500">{l.resource_type ? `${l.resource_type} #${l.resource_id}` : "—"}{l.patient_id ? ` · patient #${l.patient_id}` : ""}</span> },
          { key: "outcome", header: "Outcome", render: (l) => <StatusBadge status={l.outcome} /> },
          { key: "details", header: "Details", render: (l) => <span className="line-clamp-1 max-w-xs font-mono text-[10px] text-slate-400" title={JSON.stringify(l.details)}>{JSON.stringify(l.details)}</span> },
        ]} />
    ...
    <Card bodyClassName="p-0" title="AI query traces" subtitle="Per-request routing, latency by stage, retrieved/cited source ids, tools and tokens (query text is never stored, only its hash)">
      <DataTable dense rows={data?.items} loading={loading} error={error} onRetry={reload}
        columns={[
          ...
          { key: "stages", header: "Stages (ms)", render: (t) => <span className="font-mono text-[10px] text-slate-500">{Object.entries(t.stage_ms).map(([k, v]) => `${k}:${Math.round(v)}`).join(" ")}</span> },
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card title="Latency by stage (ms)" bodyClassName="p-0">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-slate-100 text-left text-[11px] uppercase text-slate-500"><th className="px-3 py-2">Stage</th><th>n</th><th>p50</th><th>p95</th><th>max</th></tr></thead>
            <tbody className="divide-y divide-slate-100">
        ...
        <Card title="Routes">
          <ul className="space-y-1 text-sm">{Object.entries(data.routes).map(([r, n]) => <li key={r} className="flex justify-between"><span className="flex flex-wrap gap-0.5">{r.split(" + ").map((x) => <RouteBadge key={x} route={x} />)}</span><span className="tabular-nums text-slate-600">{n}</span></li>)}</ul>
          <div className="mt-3 text-xs text-slate-500">Routing methods: {Object.entries(data.routing_methods).map(([k, v]) => `${k} ${v}`).join(" · ")}</div>
          <div className="mt-1 text-xs text-slate-500">Documents: {Object.entries(data.documents).map(([k, v]) => `${k} ${v}`).join(" · ")}</div>
          <p className="mt-3 text-xs text-slate-500">Prometheus metrics are exposed at <code>/metrics</code> on the API.</p>
        </Card>
      </div>
    </div>
  );
